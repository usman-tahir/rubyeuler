...
def pernicious(binary_string)
  binary_string.split(//).to_a.count("1").prime?
end
...
def find_first_25_pernicious_numbers
  ...
  until pernicious_numbers.count == 25
    if pernicious(number_to_binary_string(counter))
...
def find_888s
  ...
  (888_888_877..888_888_888).each do |number|
    pernicious_numbers << number if pernicious(number_to_binary_string(number))
  end
  pernicious_numbers
end

puts "First 25 pernicious numbers: #{find_first_25_pernicious_numbers.inspect}"     
puts "Pernicious numbers from ..877 to ..888 from #{find_888s.inspect}"